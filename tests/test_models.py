...
def test_ocr_result_to_dict_and_to_json_structure():
    """Verify to_dict and to_json serialize OCRResult correctly."""
    p1 = PageResult(
        page_num=1,
        markdown="Text 1",
    )
    ...
    assert dict_out["file_path"] == "sample.pdf"
    assert len(dict_out["pages"]) == 1
    assert dict_out["pages"][0]["markdown"] == "Text 1"

    json_str = result.to_json()
    assert "Text 1" in json_str
